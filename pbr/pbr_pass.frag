...
void Diagram(inout vec3 col, vec2 p, in AppState s)
{
	...
	col = mix(col, surfColor, Smooth(-r * 2.));
	...
	{
		col = mix(col, diffuseColor, Smooth(-r * 2.));
	}
...
void DrawScene(inout vec3 col, vec2 p, in AppState s)
{
    ...
    if (t > 0.0)
    {
        ...
        vec3 env1 = EnvRemap(texture(texture2, refl).xyz);
        vec3 env2 = EnvRemap(texture(texture1, refl).xyz);
        ...
        if (s.menuId == MENU_FRESNEL)
            col = envSpecularColor;
        if (s.menuId == MENU_GEOMETRY)
            col = vec3(lightV) * (4.0f * ndotv * ndotl);
...
void InfoText(inout vec3 col, vec2 p, in AppState s)
{
	...
	if (s.menuId == MENU_SPECULAR)
	{
		col = mix(col, textColor, smoothstep(.05, -.05, Capsule(q.yx - vec2(-12.3, 48.), .3, 26.)));
	}
}
...
void DrawMenuControls(inout vec3 col, vec2 p, in AppState s)
{
	p -= vec2(-110, 74);
	...
	col = mix(col, vec3(0.9), Smooth(-c2 * 2.));
